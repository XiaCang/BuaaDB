...
本章节重点展示系统核心业务逻辑在数据库层面的实现。后端通过 PyMySQL 驱动执行 SQL 语句，结合数据库的事务机制（Transaction）与触发器（Trigger），实现了数据的增删改查及完整性维护.
### 4.1 发布新商品 (数据录入)
...
* **图 4-1** 展示了前端弹出“商品发布成功”的提示。
* **图 4-2** 展示了数据库 `products` 表中新增的一条记录，可以看到 `status` 默认为 `active`，且时间字段已被正确填充。
* **[此处请插入图 4-1：前端网页截图，显示发布成功的提示]**
* **[此处请插入图 4-2：Navicat/数据库截图，按 create_time 倒序排列，展示最新插入的那条商品数据]**
...
* **图 4-3** 展示了数据库中该记录的变化。对比修改前后的数据，可以看到 `price` 和 `description` 字段已更新，且 `update_time` 变更为最新的操作时间，证明更新操作成功执行。
* **[此处请插入图 4-3：Navicat/数据库截图，展示被修改的那行数据（最好能把修改后的价格圈出来）]**
